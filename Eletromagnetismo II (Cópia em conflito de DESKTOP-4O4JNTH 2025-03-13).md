...
1. **Permissividade Elétrica $(\epsilon)$**: Mede a capacidade de um material de se polarizar em resposta a um campo elétrico. A permissividade elétrica relativa (ϵrϵr​) é a razão entre a permissividade do material e a permissividade do vácuo (ϵ0ϵ0​).
    
2. **Permeabilidade Magnética $(\mu)$**: Mede a capacidade de um material de se magnetizar em resposta a um campo magnético. A permeabilidade magnética relativa (μrμr​) é a razão entre a permeabilidade do material e a permeabilidade do vácuo (μ0μ0​).
    
3. **Índice de Refração $(n)$**: Descreve como a luz (ou outra onda eletromagnética) se propaga em um meio. Está relacionado à velocidade da luz no vácuo (cc) e à velocidade da luz no meio (vv) pela fórmula n=cvn=vc​.
    

### Relação:

Em muitos materiais dielétricos, especialmente os não magnéticos, a permeabilidade magnética relativa (μrμr​) é aproximadamente 1. Portanto, a relação simplifica-se para:
...
- Em materiais magnéticos, μrμr​ pode ser significativamente diferente de 1, e a permeabilidade magnética também influencia o índice de refração.
...
## Tamanho do Elétron  
O *elétron* é considerado uma *partícula elementar* no Modelo Padrão da física de partículas. Isso significa que, até onde se sabe experimentalmente, *não possui estrutura interna ou tamanho definido* – é tratado como um *ponto sem dimensões físicas* (raio < \(10^{-18}\) metros).  
- *Observação*: Experimentos de espalhamento de alta energia confirmam que, se o elétron tiver um "tamanho", ele é menor que \(10^{-18}\) m.
...
|---------------------------|-------------------------------------------------|  
| Tamanho do elétron        | Pontual (\(<10^{-18}\) m)                       |  
| Força de repulsão (1 m)   | \(\approx 2,3 \times 10^{-28}\) N               |  
| Dependência da distância  | \(F \propto 1/r^2\) (diminui com o quadrado de \(r\)) |  
